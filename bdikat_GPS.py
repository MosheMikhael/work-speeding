...
def get_median(value, size=21, max_memory=40, arr=[]):
...
y, x, z = img2.shape
pai_in_frame = np.array([x // 2 - 50,  y * 0.4], int)
Width  = img2.shape[1]
Height = img2.shape[0]
fs = 1
...
times_GPS = [None for i in range(frame_delay)]
...
# for skiping video
    ...
    max_y = []
    if len(B_median7) != 0:
        _B_ = B_median7[len(B_median7) - 1]
    for itt_ in range(Width):
        _x_ = int(itt_ - pai_in_frame[0])
        max_y.append(int(mm_of_movement.maximum_figure(_x_, _B_, B_factor, max_OF=winSize_y, H=H, L=L)))
    ...
    if debug:
        ...
        if len(B_median7) != 0:
            _B_ = B_median7[len(B_median7) - 1]
        for itt_ in range(Width):
            out = draw.draw_point(out, (int(itt_), int(max_y[itt_] + pai_in_frame[1])), 1, color=draw.cyan)
    ...
    all_points, count_of_interesting_points = sf.get_points(lenght_of_axis, np.pi / 2 - al, bt - np.pi / 2 ,LINES, max_y, pai_in_frame) # generating vector of all points
...
#OPTICAL FLOW
    ...
    try:
        pts1, pts2 = pai_module.find_opt_flow_lk_with_points(img1, img2, mod_pts, winSize_x, winSize_y)
    except:
        make_result(out, frame_itt, time_start_itt, tmp_map, times_g[frame_itt + delta_fr], (lon_g[frame_itt + delta_fr], lat_g[frame_itt + delta_fr]), speed_g[frame_itt + delta_fr], 
    ...
    while len(Ba) + 1 != len(Ta):
        Ba.append(None)
    Ba.append(B_v)
    Bm7_v = get_median(np.abs(B_v))
    while len(B_median7) + 1 != len(Ta):
        B_median7.append(None)
    B_median7.append(Bm7_v)
    make_result(out, frame_itt, time_start_itt, tmp_map, times_g[frame_itt + delta_fr], (lon_g[frame_itt + delta_fr], lat_g[frame_itt + delta_fr]), speed_g[frame_itt + delta_fr], 
                residuals, A_x, A_y, A_z, B, ofSpeed=B_v, medianSpeed=Bm7_v, debug=debug, debug_graph=debug_graph)
    if debug_graph:
        t0 = time.time()
        fig_B, ax_B = plt.subplots()
        ax_B.set_title('speed')
